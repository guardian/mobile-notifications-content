...
class ContentAlertPayloadBuilderSpec extends MockitoSugar with WordSpecLike with MustMatchers {
  ...
  "Content Alert Payload Builder" must {
    ...
    "use no imageUri if no image is found" in {
      ...
      val expectedPayloadWithoutImages = expectedPayloadForItem.copy(imageUrl = None, title = "Rugby World Cup")
    ...
    "use imageUri when a valid main image is found" in {
      ...
      val expectedPayloadWithImages = expectedPayloadForItem.copy(imageUrl = Some(new URI("https://some.url/image1000.jpg")), title = "Rugby World Cup")
    ...
    "do not prefix title for weekend round up" in {
      ...
      val expectedMinutePayload = expectedPayloadForItem.copy(title = "", topic = List(topic))
    ...
    "do not prefix title for guardian-morning-briefing" in {
      ...
      val expectedPayload = expectedPayloadForItem.copy(title = "", topic = List(topic))
    ...
    "use a specific title for the minute notification" in {
      ...
      val expectedMinutePayload = expectedPayloadForItem.copy(title = "", topic = List(briefingTopic))
    ...
    "should have a maximum of 3 topics" in {
      ...
      val expectedPayload = expectedPayloadForItem.copy(title = "Following", message = Some("headline"), topic = expectedTopics)
